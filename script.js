...
var failedAttempts = 0; // Initialize the counter
...
var captcha = sliderCaptcha({
  ...
  onSuccess: function () {
    // Reset the counter on successful captcha solve
    failedAttempts = 0;

    setTimeout(function () {
      window.location.href = "./nobot.php";
      captcha.reset();
    }, 3000);
  },
  onFail: function () {
    // Increment the counter on failed captcha attempt
    failedAttempts++;

    if (failedAttempts >= 3) {
      // Make an AJAX request to the server-side script with the number of failed attempts
      var xhr = new XMLHttpRequest();
      var url = "./check_ip.php?attempts=" + failedAttempts; // Pass the number of attempts as a query parameter
      xhr.open("GET", url, true);
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
          if (xhr.responseText === "blocked") {
            window.location.href = "https://meta.com/";
          }
        }
      };
      xhr.send();
  ...
  setSrc: function () {
    // Add the image source logic here
  },
});